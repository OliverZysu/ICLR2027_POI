"""Train/evaluate iPCM using only the host project's existing ``utils``."""
...
from baselines.ipcm.model import IPCM
...
def build_context(train_df: pd.DataFrame, mappings, processed_dir: Path, args) -> Dict:
    ...
    cache_path = processed_dir / (".ipcm_context_%s.pt" % fingerprint)
    ...
    region_rows = poi_region[torch.tensor(pois, dtype=torch.long)].numpy()
    context = {
        ...
        "region_poi_prior": _dense_prior(
            region_rows, pois, (num_regions, num_pois)
        ),
        "metadata": {
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="iPCM Next-POI baseline")
...
def make_model(mappings, args, context: Dict) -> IPCM:
    return IPCM(
        num_users=mappings.num_users,
        num_pois=mappings.num_pois,
...
def run_experiment(args: argparse.Namespace) -> Dict:
    ...
    checkpoint_dir = ensure_dir(PROJECT_ROOT / "checkpoints" / "iPCM" / run_tag)
    result_dir = ensure_dir(PROJECT_ROOT / "results" / "iPCM")
    ...
    print(
        "model=iPCM city=%s device=%s users=%d pois=%d train/val/test=%d/%d/%d"
        % (
            city,
            device,
            mappings.num_users,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        total_aux = 0.0
        batches = 0
        progress = tqdm(train_loader, desc="iPCM-%s ep%d" % (city, epoch), leave=False)
        ...
        print(
            "[iPCM-%s] epoch=%d loss=%.6f main=%.6f aux=%.6f val=[%s loss=%.6f]"
            % (
                city,
                epoch,
                record["train_loss"],
    ...
    if best_validation is None or not checkpoint_path.exists():
        raise RuntimeError("iPCM did not produce a valid checkpoint")
    ...
    print("[iPCM-%s] TEST %s" % (city, format_metrics(test_metrics)))
