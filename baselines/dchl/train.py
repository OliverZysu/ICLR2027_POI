"""Train/evaluate DCHL using only the host project's existing ``utils``."""
...
from baselines.dchl.model import DCHL
...
def build_context(train_df: pd.DataFrame, mappings, processed_dir: Path, args) -> Dict:
    ...
    cache_path = processed_dir / (".dchl_context_%s.pt" % fingerprint)
    ...
    geo_indices, geo_scores = _geo_neighbors(mappings, args.geo_k)
    geo_rows = []
    geo_columns = []
    geo_values = []
    for source in range(num_pois):
        for destination, score in zip(geo_indices[source], geo_scores[source]):
            geo_rows.extend([source, int(destination)])
            geo_columns.extend([int(destination), source])
            geo_values.extend([float(score), float(score)])
    geo_graph = _normalised_sparse(
        geo_rows, geo_columns, geo_values, (num_pois, num_pois)
    )
    ...
    transition_rev = _normalised_sparse(
        transition_columns,
        transition_rows,
        transition_values,
        (num_pois, num_pois),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="DCHL Next-POI baseline")
    add_common_args(parser)
    parser.add_argument("--embedding-dim", type=int, default=64)
    parser.add_argument("--context-dim", type=int, default=24)
    parser.add_argument("--num-factors", type=int, default=4)
    parser.add_argument("--hyper-layers", type=int, default=1)
    ...
    parser.add_argument("--max-contrastive-samples", type=int, default=384)
...
def make_model(mappings, args, context: Dict) -> DCHL:
    return DCHL(
        num_users=mappings.num_users,
        num_pois=mappings.num_pois,
        ...
        max_contrastive_samples=args.max_contrastive_samples,
    )
...
def run_experiment(args: argparse.Namespace) -> Dict:
    ...
    checkpoint_dir = ensure_dir(PROJECT_ROOT / "checkpoints" / "DCHL" / run_tag)
    result_dir = ensure_dir(PROJECT_ROOT / "results" / "DCHL")
    ...
    print(
        "model=DCHL city=%s device=%s users=%d pois=%d train/val/test=%d/%d/%d"
        % (
            city,
            device,
            mappings.num_users,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        total_aux = 0.0
        batches = 0
        progress = tqdm(train_loader, desc="DCHL-%s ep%d" % (city, epoch), leave=False)
        ...
        print(
            "[DCHL-%s] epoch=%d loss=%.6f main=%.6f aux=%.6f val=[%s loss=%.6f]"
            % (
                city,
                epoch,
                record["train_loss"],
    ...
    if best_validation is None or not checkpoint_path.exists():
        raise RuntimeError("DCHL did not produce a valid checkpoint")
    ...
    print("[DCHL-%s] TEST %s" % (city, format_metrics(test_metrics)))
