"""Train/evaluate K1-POI using only the host project's existing ``utils``."""
...
from baselines.k1_poi.model import K1POI
...
def build_context(train_df: pd.DataFrame, mappings, processed_dir: Path, args) -> Dict:
    ...
    cache_path = processed_dir / (".k1_poi_context_%s.pt" % fingerprint)
    ...
    for _, trajectory in work.groupby("trajectory_id", sort=False):
        ...
        for source, destination in zip(sequence[:-1], sequence[1:]):
            transition_rows[int(source)][int(destination)] += 1
    transition_top_idx, transition_top_score = _topk_rows(
        transition_rows, num_pois, args.prior_k
    )
    geo_idx, geo_score = _geo_neighbors(mappings, args.geo_k)

    users = work["user_idx"].astype(int).to_numpy()
    pois = work["poi_idx"].astype(int).to_numpy()
    time_rows = (
        work["weekday"].astype(int).to_numpy() * 24
        + work["hour"].astype(int).to_numpy()
    )
    global_counts = np.bincount(pois, minlength=num_pois).astype(np.float32)
    global_prior = np.log1p(global_counts)
    global_prior = global_prior / max(float(global_prior.max()), 1e-6)

    context = {
        "transition_top_idx": transition_top_idx,
        "transition_top_score": transition_top_score,
        "geo_top_idx": torch.tensor(geo_idx, dtype=torch.long),
        "geo_top_score": torch.tensor(geo_score, dtype=torch.float32),
        "user_poi_prior": _dense_prior(users, pois, (num_users, num_pois)),
        "time_poi_prior": _dense_prior(time_rows, pois, (168, num_pois)),
        "global_poi_prior": torch.tensor(global_prior, dtype=torch.float32),
        ...
        "metadata": {
            ...
            "geo_k": int(geo_idx.shape[1]),
            "transition_k": int(transition_top_idx.shape[1]),
        },
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="K1-POI Next-POI baseline")
    ...
    parser.add_argument("--recall-loss-weight", type=float, default=0.25)
    parser.add_argument("--num-regions", type=int, default=64)
    parser.add_argument("--geo-k", type=int, default=32)
    parser.add_argument("--prior-k", type=int, default=64)
...
def make_model(mappings, args, context: Dict) -> K1POI:
    return K1POI(
        num_users=mappings.num_users,
        num_pois=mappings.num_pois,
...
def run_experiment(args: argparse.Namespace) -> Dict:
    ...
    checkpoint_dir = ensure_dir(PROJECT_ROOT / "checkpoints" / "K1-POI" / run_tag)
    result_dir = ensure_dir(PROJECT_ROOT / "results" / "K1-POI")
    ...
    print(
        "model=K1-POI city=%s device=%s users=%d pois=%d train/val/test=%d/%d/%d"
        % (
            city,
            device,
            mappings.num_users,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        total_aux = 0.0
        batches = 0
        progress = tqdm(train_loader, desc="K1-POI-%s ep%d" % (city, epoch), leave=False)
        ...
        print(
            "[K1-POI-%s] epoch=%d loss=%.6f main=%.6f aux=%.6f val=[%s loss=%.6f]"
            % (
                city,
                epoch,
                record["train_loss"],
    ...
    if best_validation is None or not checkpoint_path.exists():
        raise RuntimeError("K1-POI did not produce a valid checkpoint")
    ...
    print("[K1-POI-%s] TEST %s" % (city, format_metrics(test_metrics)))
