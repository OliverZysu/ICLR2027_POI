"""Train/evaluate MTNet using only the host project's existing ``utils``."""
...
from baselines.mtnet.model import MTNet
...
def _mapping_signature(mappings) -> str:
    ...
    return hashlib.sha1("\0".join(item[0] for item in ordered).encode("utf-8")).hexdigest()[:16]


def _training_poi_indices(train_df: pd.DataFrame, mappings) -> np.ndarray:
    indices = [
        mappings.poi_id2idx[_canonical(value)]
        for value in train_df["POI_id"].tolist()
        if _canonical(value) in mappings.poi_id2idx
    ]
    if not indices:
        return np.arange(mappings.num_pois, dtype=np.int64)
    return np.unique(np.asarray(indices, dtype=np.int64))


def _build_regions(train_df: pd.DataFrame, mappings, num_regions: int, seed: int) -> torch.Tensor:
    ...
    train_indices = _training_poi_indices(train_df, mappings)
...
def build_context(train_df: pd.DataFrame, mappings, processed_dir: Path, args) -> Dict:
    ...
    cache_path = processed_dir / (".mtnet_context_%s.pt" % fingerprint)
    ...
    poi_region = _build_regions(train_df, mappings, args.num_regions, args.seed)
    poi_category = torch.zeros(mappings.num_pois, dtype=torch.long)
    for poi_idx, category_idx in mappings.poi_idx2cat_idx.items():
        poi_category[int(poi_idx)] = int(category_idx)
    context = {
        ...
        "metadata": {
            ...
            "num_pois": int(mappings.num_pois),
        },
    }
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="MTNet Next-POI baseline")
    ...
    parser.add_argument("--category-loss-weight", type=float, default=0.3)
    parser.add_argument("--region-loss-weight", type=float, default=0.3)
...
def make_model(mappings, args, context: Dict) -> MTNet:
    return MTNet(
        num_users=mappings.num_users,
        num_pois=mappings.num_pois,
        num_categories=mappings.num_categories,
        poi_region=context["poi_region"],
...
def run_experiment(args: argparse.Namespace) -> Dict:
    ...
    checkpoint_dir = ensure_dir(PROJECT_ROOT / "checkpoints" / "MTNet" / run_tag)
    result_dir = ensure_dir(PROJECT_ROOT / "results" / "MTNet")
    ...
    print(
        "model=MTNet city=%s device=%s users=%d pois=%d train/val/test=%d/%d/%d"
        % (
            city,
            device,
            mappings.num_users,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        total_aux = 0.0
        batches = 0
        progress = tqdm(train_loader, desc="MTNet-%s ep%d" % (city, epoch), leave=False)
        ...
        print(
            "[MTNet-%s] epoch=%d loss=%.6f main=%.6f aux=%.6f val=[%s loss=%.6f]"
            % (
                city,
                epoch,
                record["train_loss"],
    ...
    if best_validation is None or not checkpoint_path.exists():
        raise RuntimeError("MTNet did not produce a valid checkpoint")
    ...
    print("[MTNet-%s] TEST %s" % (city, format_metrics(test_metrics)))
